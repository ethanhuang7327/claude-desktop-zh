...
BACKUP_GLOB = "Claude.backup-before-zh-*.app"
...
def patch_language_whitelist(app: Path, lang_codes: list[str]) -> Path:
    ...
    unique_langs = list(dict.fromkeys(lang_codes))
    replacement = BASE_LANGUAGE_LIST + "".join(f',"{lang}"' for lang in unique_langs) + "]"

    for path in candidates:
        text = path.read_text(encoding="utf-8")
        if all(f'"{lang}"' in text for lang in unique_langs):
            print(f"Language whitelist already contains {', '.join(unique_langs)}: {path.name}")
            return path
        if LANG_LIST_RE.search(text):
            ...
            print(f"Patched language whitelist for {', '.join(unique_langs)}: {path.name}")
            return path
...
def install_language_resources(app: Path, lang_code: str) -> None:
    merge_frontend_locale(app, lang_code)
...
def backup_and_replace(original: Path, patched: Path, lang_code: str, dry_run: bool) -> Path:
    stamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    backup = original.with_name(f"Claude.backup-before-{lang_code}-{stamp}.app")
    if dry_run:
...
def resolve_backup(app: Path, backup: str) -> Path:
    if backup != "latest":
        path = Path(backup).expanduser()
        if not path.is_dir():
            raise SystemExit(f"Backup does not exist or is not a directory: {path}")
        return path

    backups = find_app_backups(app)
    if not backups:
        raise SystemExit(f"No Claude backup found in {app.parent}: {BACKUP_GLOB}")
    return sorted(backups, key=lambda item: item.stat().st_mtime, reverse=True)[0]


def restore_backup(app: Path, backup_arg: str, dry_run: bool) -> Path:
    backup = resolve_backup(app, backup_arg)
    stamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
...
def main() -> int:
    ...
    parser.add_argument("--lang", choices=["zh-CN", "zh-TW", "zh-HK"], default="zh-CN", help="Primary language code to install and set as current locale")
    parser.add_argument("--langs", help="Comma-separated language codes to install, for example zh-CN,zh-TW")
    ...
    parser.add_argument("--restore", action="store_true", help="Restore a macOS app backup")
    parser.add_argument("--backup", default="latest", help="Backup to restore: latest or an explicit .app path")
    parser.add_argument("--advanced-asar-patch", action="store_true", help="Patch app.asar for 3P gateway model-name validation and main-process menu labels")
    ...
    if args.restore:
        if args.dry_run:
            ...
            quit_claude()
        restored = restore_backup(args.app, args.backup, args.dry_run)
        if args.dry_run:
        ...
        print("Done. Claude Desktop has been restored from backup.")
        return 0

    lang_code = args.lang
    lang_codes = [item.strip() for item in (args.langs.split(",") if args.langs else [lang_code]) if item.strip()]
    for item in lang_codes:
        if item not in {"zh-CN", "zh-TW", "zh-HK"}:
            raise SystemExit(f"Unsupported language code: {item}")
    if lang_code not in lang_codes:
        lang_codes.insert(0, lang_code)
    config = get_language_config(lang_code)
    label = config["label"]

    for item in lang_codes:
        item_config = get_language_config(item)
        require_file(item_config["frontend_translation"])
        require_file(item_config["frontend_hardcoded"])
        require_file(item_config["desktop_translation"])
        require_file(item_config["localizable_strings"])
    ...
    if args.advanced_asar_patch:
        ...
        print("Safe mode: skipping main-process menu label patch (app.asar)")
    if args.advanced_asar_patch:
        patch_custom3p_model_validation(patched_app)
    else:
        print("Safe mode: skipping 3P model validation patch (app.asar)")
    for item in lang_codes:
        install_language_resources(patched_app, item)
    ...
    for item in lang_codes:
        verify(patched_app, item)

    backup = backup_and_replace(args.app, patched_app, lang_code, args.dry_run)
    if not args.dry_run:
