...
function Read-InteractiveSelection {
    ...
    Write-Host ""
    Write-Host "[1] 安装中文语言包（默认安全模式，不修改 app.asar）"
    Write-Host "[2] 安装中文语言包 + Advanced app.asar 补丁"
    Write-Host "[3] 恢复原样 / 卸载补丁"
    Write-Host "[Q] 退出"
    Write-Host ""

    $advancedAsarPatchForInstall = $false
    $actionSelected = $false
    while (-not $actionSelected) {
        $actionSelection = (Read-Host "请选择操作 [1/2/3/Q]").Trim()
        switch -Regex ($actionSelection) {
            '^[1]$' { $advancedAsarPatchForInstall = $false; $actionSelected = $true }
            '^[2]$' { $advancedAsarPatchForInstall = $true; $actionSelected = $true }
            '^[3]$' { return @{ Action = "uninstall"; Language = "zh-CN"; AdvancedAsarPatch = $false } }
            '^[Qq]$' { exit 0 }
    ...
    while ($true) {
        ...
        switch -Regex ($languageSelection) {
            '^[1]$' { return @{ Action = "install"; Language = "zh-CN"; AdvancedAsarPatch = $advancedAsarPatchForInstall } }
            '^[2]$' { return @{ Action = "install"; Language = "zh-TW"; AdvancedAsarPatch = $advancedAsarPatchForInstall } }
            '^[3]$' { return @{ Action = "install"; Language = "zh-HK"; AdvancedAsarPatch = $advancedAsarPatchForInstall } }
            '^[Qq]$' { exit 0 }
            default { Write-Host "请输入 1、2、3 或 Q。" -ForegroundColor Yellow }
        }
    }
...
if ($Interactive) {
    ...
    if ($interactiveSelection.AdvancedAsarPatch) {
        $AdvancedAsarPatch = $true
    }
}
...
function Install-LanguageFiles {
    ...
    Merge-LocaleFile (Join-Path $i18nDir "en-US.json") $Pack["Frontend"] (Join-Path $i18nDir "$Lang.json")
    Write-Host "  installed ion-dist/i18n/$Lang.json" -ForegroundColor Green

    Merge-LocaleFile (Join-Path $ResourcesPath "en-US.json") $Pack["Desktop"] (Join-Path $ResourcesPath "$Lang.json")
    Write-Host "  installed resources/$Lang.json" -ForegroundColor Green

    $statsigEn = Join-Path $statsigDir "en-US.json"
    if (Test-Path $statsigEn -PathType Leaf) {
        Merge-LocaleFile $statsigEn $Pack["Statsig"] (Join-Path $statsigDir "$Lang.json")
    }
    else {
...
function Merge-LocaleFile {
    ...
    Require-File $TranslationPath

    $english = Get-Content $EnglishPath -Raw -Encoding UTF8 | ConvertFrom-Json
    $translated = Get-Content $TranslationPath -Raw -Encoding UTF8 | ConvertFrom-Json
    $merged = [ordered]@{}
    $translatedCount = 0
    $fallbackCount = 0

    foreach ($property in $english.PSObject.Properties) {
        $match = $translated.PSObject.Properties[$property.Name]
        if ($match) {
            $merged[$property.Name] = $match.Value
            if ($match.Value -ne $property.Value) {
                $translatedCount += 1
            }
        }
        else {
    ...
    Write-Host "  merged locale: $translatedCount translated, $fallbackCount fallback" -ForegroundColor DarkGray
}
...
function Disable-CompressedAssetCache {
    param([string]$ResourcesPath)

    $assetsDir = Join-Path $ResourcesPath "ion-dist\assets\v1"
    if (-not (Test-Path $assetsDir -PathType Container)) {
        Write-Host "  [警告] 未找到 assets 目录，跳过 .zst 处理。" -ForegroundColor DarkYellow
        return
    }

    $renamed = 0
    $assets = @()
    $assets += @(Get-ChildItem (Join-Path $assetsDir "*.js") -File -ErrorAction SilentlyContinue)
    $assets += @(Get-ChildItem (Join-Path $assetsDir "*.css") -File -ErrorAction SilentlyContinue)
    foreach ($asset in $assets) {
        $compressed = "$($asset.FullName).zst"
        if (-not (Test-Path $compressed -PathType Leaf)) {
            continue
        }
        $backup = "$compressed.bak"
        if (Test-Path $backup -PathType Leaf) {
            continue
        }

        Backup-ModifiedFile $ResourcesPath $compressed
        Rename-Item -LiteralPath $compressed -NewName ([System.IO.Path]::GetFileName($backup)) -Force
        $renamed += 1
    }

    Write-Host "  disabled compressed asset cache: $renamed .zst file(s)" -ForegroundColor Green
}
...
function Install-WindowsLanguagePack {
    ...
    Disable-CompressedAssetCache $resourcesPath
    if ($AdvancedAsarPatch) {
        Patch-HardcodedMainProcessMenuLabels $resourcesPath $LanguageCode
    } else {
        Write-Host "  safe mode: skipping main-process menu label patch (app.asar)" -ForegroundColor DarkYellow
    }
    ...
    if ($AdvancedAsarPatch) {
        ...
        Write-Host "  safe mode: skipping 3P model validation patch (app.asar)" -ForegroundColor DarkYellow
    }

    if (-not $AdvancedAsarPatch) {
        Write-Host "  safe mode: skipping Claude.exe asar integrity sync" -ForegroundColor DarkYellow
    }
...
function Uninstall-WindowsLanguagePack {
    ...
    try {
        ...
        Write-Host "  [警告] app.asar integrity sync skipped: $($_.Exception.Message)" -ForegroundColor DarkYellow
    }
